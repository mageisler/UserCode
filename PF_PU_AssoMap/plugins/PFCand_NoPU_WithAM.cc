...
// 
/**\class PF_PU_AssoMap PFCand_NoPU_WithAM.cc MGeisler/PF_PU_AssoMap/src/PFCand_NoPU_WithAM.cc
...
//
//
#include "MGeisler/PF_PU_AssoMap/interface/PFCand_NoPU_WithAM.h"
...
PFCand_NoPU_WithAM::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	//get the input vertex collection
 	...
 	VertexRefV* qualvtxcoll = PFCand_NoPU_WithAM_Algos::QualifiedVertices(vtxcollH,true,4.);
	  ...
	  if(PFCtrackref.isNull()){
	    ...
	    PFDisplacedVertex displVtx;
       	    ...
       	    switch( candptr->particleId() ){
       	      case PFCandidate::h:{
		cout << "h" << endl;
	        if(PFCand_NoPU_WithAM_Algos::ComesFromV0Decay(candptr,vertCompCandCollKshortH,vertCompCandCollLambdaH,qualvtxcoll,&vtxref_tmp)){
	        ...
	        if(PFCand_NoPU_WithAM_Algos::ComesFromNI(candptr,displVertexCollH,&displVtx,iSetup))
	          if(PFCand_NoPU_WithAM_Algos::FindNIVertex(candptr,displVtx,qualvtxcoll,true,iSetup)==AMfirstvertex) firstvertexCandidates->push_back(*candptr);
                break;
	      }
       	      case PFCandidate::gamma:{
		cout << "gamma" << endl; 
	        if(PFCand_NoPU_WithAM_Algos::FindPFCandVertex(candptr,qualvtxcoll,iSetup)==AMfirstvertex){
		  cout << "YES" << endl; 
	          firstvertexCandidates->push_back(*candptr);
                ...
                break;
       	      ...
       	      case PFCandidate::e:{
		cout << "e" << endl;
	        if(PFCand_NoPU_WithAM_Algos::ComesFromConversion(candptr,convCollH,qualvtxcoll,&vtxref_tmp))
	          if(vtxref_tmp==AMfirstvertex) firstvertexCandidates->push_back(*candptr);
                break;
	      }
       	      case PFCandidate::mu:{
	        cout << "mu" << endl;
	        if(PFCand_NoPU_WithAM_Algos::FindPFCandVertex(candptr,qualvtxcoll,iSetup)==AMfirstvertex) firstvertexCandidates->push_back(*candptr);
                break;
	      }
       	      case PFCandidate::h0:{
		cout << "h0" << endl;
